...
int run_day5_part2(char* line, int rule_list[2000][2], int rule_count,
                   int updates[500], int updates_count) {
...
int run_day5_part1(char* line, int rule_list[2000][2], int rule_count,
                   int updates[500], int updates_count) {
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    if (input_part == UPDATES) {
      ...
      int line_result_part1 =
          run_day5_part1(line, rule_list, rule_count, updates, updates_count);
      result_part_1 = result_part_1 + line_result_part1;
      if (line_result_part1 == 0) {
        int line_result_part2 =
            run_day5_part2(line, rule_list, rule_count, updates, updates_count);
