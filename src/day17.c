...
long long int combo_operand(struct Register state, long long int operand) {
	if (operand == 4) {
		return state.A;
	} else if (operand == 5) {
		return state.B;
...
// --- INSTRUCTIONS ---
struct Register adv(struct Register state, long long int operand) {
...
struct Register bxl(struct Register state, long long int operand) {
	long long int result = state.B ^ operand;
	struct Register new_state = { state.A, result, state.C};
	return new_state;
}
struct Register bst(struct Register state, long long int operand) {
...
struct Register bxc(struct Register state) {
...
struct Register bdv(struct Register state, long long int operand) {
	long long int numerator = state.A;
	long long int denumerator = power(2, combo_operand(state, operand));
	long long int result = numerator / denumerator;
	// todo truncated to integer
...
struct Register cdv(struct Register state, long long int operand) {
	long long int numerator = state.A;
	long long int denumerator = power(2, combo_operand(state, operand));
	long long int result = numerator / denumerator;
	// todo truncated to integer
...
struct OutputArray run_program(struct Register initialState, struct OutputArray program) {
	...
	while (program_cursor < program.size) {
		state = run_opcode(program.arr[program_cursor], program.arr[program_cursor + 1], state, &program_cursor, &output);
	}
...
struct OutputArray run_program_with_limit(struct Register initialState, struct OutputArray program) {
	int program_cursor = 0;
	struct Register state = initialState;
	struct OutputArray output = {{0}, 0};
