...
int day14part2(struct ToiletGuard guard_list[500], int guard_count) {
  ...
  while (seconds < 100000000) {
    ...
    for (int i = 0; i < guard_count; i++) {
      int x = (guard_list[i].start_x + (guard_list[i].velocity_x * seconds)) %
              width;
      if (x < 0) {
        x = width + x;
      }
      ...
      int y = (guard_list[i].start_y + (guard_list[i].velocity_y * seconds)) %
              height;
      if (y < 0) {
        y = height + y;
      }
      ...
      matrix[guard_list[i].y][guard_list[i].x] =
          matrix[guard_list[i].y][guard_list[i].x] + 1;
    }
...
int day14part1(struct ToiletGuard guard_list[500], int guard_count) {
  ...
  for (int i = 0; i < guard_count; i++) {
    // int i = 10;
    int x =
        (guard_list[i].start_x + (guard_list[i].velocity_x * seconds)) % width;
    if (x < 0) {
      x = width + x;
    }
    guard_list[i].x = x;
    ...
    int y =
        (guard_list[i].start_y + (guard_list[i].velocity_y * seconds)) % height;
    if (y < 0) {
      y = height + y;
    }
    guard_list[i].y = y;
...
int run_day14() {
  ...
  while ((read = getline(&line, &len, fp)) != -1) {
    ...
    char *vel_y =
        get_substring(velocity_comma + 1, read - 3 - position_comma, velocity);
