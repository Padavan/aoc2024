...
bool move_robot_rec(char** matrix, size_t position[2], int direction) {
  ...
  if (matrix[next[0]][next[1]] == 'O' || matrix[next[0]][next[1]] == '[' ||
...
size_t get_gps_sum(char** matrix, size_t height, size_t width) {
  size_t result = 0;
...
size_t day15part1(char** matrix, size_t size, int moves[30000],
                  size_t moves_count) {
...
void fat_matrix_flood(char** matrix, size_t position[2],
                      enum DIRECTION direction, size_t points[][2],
                      size_t* points_count, bool* can_move) {
  ...
  if (matrix[next[0]][next[1]] == '[') {
    ...
    fat_matrix_flood(matrix, next_adj, direction, points, points_count,
                     can_move);
...
void move_fat_flood(char** matrix, size_t points[][2], size_t index,
                    size_t length, enum DIRECTION direction) {
...
size_t day15part2(char** matrix, size_t height, size_t width, int moves[30000],
                  size_t moves_count) {
  ...
  for (size_t i = 0; i < moves_count; i++) {
    ...
    if (direction == LEFT || direction == RIGHT) {
      ...
      bool can_move = true;
      fat_matrix_flood(matrix, robot_pos, direction, points, &points_count,
                       &can_move);
...
int run_day15() {
  ...
  printf("\tPart 2: %ld\n", day15part2(fat_matrix, matrix_width,
                                       matrix_width * 2, moves, moves_count));

  for (size_t i = 0; i < matrix_width; ++i) free(matrix[i]);
  free(matrix);

  for (size_t i = 0; i < matrix_width; ++i) free(fat_matrix[i]);
  free(fat_matrix);
