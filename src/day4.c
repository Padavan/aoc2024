...
int check_point_x(int point[2], char** matrix, int width, int height) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    _Bool is_m = is_inbound(second, width, height) &&
                 matrix[second[1]][second[0]] == 'M';
    _Bool is_a =
        is_inbound(third, width, height) && matrix[third[1]][third[0]] == 'A';
    _Bool is_s = is_inbound(fourth, width, height) &&
...
int check_point_a(int point[2], char** matrix, int width, int height) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    _Bool is_m2 = is_inbound(second, width, height) &&
                  matrix[second[1]][second[0]] == 'M';
    _Bool is_s1 =
        is_inbound(third, width, height) && matrix[third[1]][third[0]] == 'S';
    _Bool is_s2 = is_inbound(fourth, width, height) &&
                  matrix[fourth[1]][fourth[0]] == 'S';
