...
size_t near_diagonal_areas[4][2] = { { 1, 1 },
    { -1, 1 },
    { -1, -1 },
    { 1, -1 } };

void get_area_points(char** matrix,
    size_t width,
    size_t height,
    size_t target[2],
    ...
    size_t* corners)
{
    ...
    *area_points_count = *area_points_count + 1;
    ...
    for (size_t i = 0; i < 4; i++) {
        ...
        if (is_inbound(side, width, height) && matrix[target[0]][target[1]] == matrix[side[0]][side[1]]) {
            shit[i] = 1;
        }

        if (is_inbound(side, width, height) && matrix[target[0]][target[1]] == matrix[side[0]][side[1]] && !arr_includes(area_points, *area_points_count, side)) {
            get_area_points(
                matrix, width, height, side, area_points, area_points_count, corners);
        }
...
// super expensive
size_t
get_perimeter(size_t area_points[500][2],
    size_t* area_points_count,
    size_t width,
    size_t height)
{
...
struct Day12Result
analyze_day12_matrix(char** matrix, size_t width, size_t height)
{
    ...
    for (size_t row = 0; row < height; row++) {
        for (size_t col = 0; col < width; col++) {
            if (find_character_index(CAPITAL_ALPHABET, matrix[row][col]) > -1) {
                ...
                size_t perimeter = get_perimeter(area_points, &area_points_count, width, height);
...
int run_day12()
{
    ...
    struct Day12Result result = analyze_day12_matrix(matrix, matrix_width, matrix_height);
    printf("\tPart 1: %ld\n", result.a);
    printf("\tPart 2: %ld\n", result.b);
