...
long long
get_b(long long a, struct Machine machine)
{
    assert(machine.a_x > 0 && machine.b_y > 0 && machine.a_y > 0 && machine.b_x > 0);
    return (machine.x_loc - (a * machine.a_x)) / machine.b_x;
}

long long
get_a(struct Machine machine)
{
    assert(machine.a_x > 0 && machine.b_y > 0 && machine.a_y > 0 && machine.b_x > 0);
    return (machine.x_loc * machine.b_y - machine.y_loc * machine.b_x) / ((machine.a_x * machine.b_y) - (machine.a_y * machine.b_x));
};

bool check_machine(struct Machine machine, long long a, long long b)
{
    if (a * machine.a_x + b * machine.b_x == machine.x_loc && a * machine.a_y + b * machine.b_y == machine.y_loc) {
        return true;
    } else {
        return false;
    }
...
long long
day13part1(struct Machine list[500], int machine_count)
{
...
long long
day13part2(struct Machine list[500], int machine_count)
{
    long long token_sum = 0;
