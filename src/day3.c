...
enum STATES
step_state(enum STATES state, char current)
{
...
enum D_STATE
build_switch(char checked_char, char match_char, enum D_STATE next_state)
{
...
enum D_STATE
d_step_state(enum D_STATE state, char current)
{
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        for (int i = 0; i < read; i++) {
            ...
            if (current_state == END) {
                ...
                char* second_number_str = get_substring(
                    comma_index + 1, strlen(multiplier_str), multiplier_str);
