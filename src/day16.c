...
bool flood_with_cap(size_t** matrix,
    size_t size,
    struct Point target,
    struct Point end,
    enum DIRECTION direction,
    ...
    size_t cap)
{
    ...
    if (!is_next_cwcell_wall && is_next_cw_points_highter) {
        matrix[cw_next.row][cw_next.col] = current_score + 1001;
        is_next_cwcell_best = flood_with_cap(
            matrix, size, cw_next, end, cw_direction, tiles, count, cap);
        if (is_next_cwcell_best) {
            matrix[target.row][target.col] = current_score + 1000;
        }
    }
    ...
    if (!is_ccwcell_wall && is_ccwcell_higher_points) {
        matrix[ccw_next.row][ccw_next.col] = current_score + 1001;
        is_ccwcell_best = flood_with_cap(
            matrix, size, ccw_next, end, ccw_direction, tiles, count, cap);
...
struct DAY16Result
get_lowest_labyrint_score(size_t** matrix,
    size_t size,
    size_t start[2],
    size_t end[2])
{
